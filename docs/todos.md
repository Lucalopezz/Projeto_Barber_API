...
- [ ] **Criar uma vitrine pública de barbearias e serviços.** `GET /barber-shop` e `GET /barber-shop/:id` exigem JWT, enquanto o cliente precisa explorar estabelecimentos. Principalmente, `GET /services` só encontra a barbearia do dono autenticado e não aceita `barberShopId`; portanto, após escolher uma barbearia o front não consegue obter seus serviços. Proposta: `GET /barber-shops` e `GET /barber-shops/:barberShopId/services`, públicos ou acessíveis a qualquer usuário autenticado.
- [ ] **Padronizar e expor os IDs relacionais necessários.** A navegação precisa de `barberShop.id` → `service.id` → `appointment.id`. Corrigir o campo de resposta de serviço: `barberShopOwnerId` contém, na verdade, o `barberShopId`. Expor o nome correto e documentar uma estratégia de migração/compatibilidade para o front.
- [ ] **Disponibilizar uma rota de contexto do usuário.** `GET /users/get-one` retorna só os dados básicos do usuário; ele não retorna `barberShopId` nem a barbearia do dono. Criar `GET /users/me` com `id`, `role`, `barberShop` (quando houver) e permissões/contexto. Isso elimina buscas indiretas e estado duplicado no front.
- [ ] **Corrigir a autorização dos agendamentos.** `PATCH /appointments/:id` e `PUT /appointments/:id` validam somente se o usuário possui alguma barbearia; não confirmam que o agendamento pertence àquela barbearia. Um barbeiro pode alterar o agendamento de outra. Validar `appointment.barberShopId === barberShop.id` e devolver `403` quando não houver permissão.
