...
describe('UpdateAppointmentUseCase integration tests', () => {
  ...
  it('should update appointment date successfully', async () => {
    ...
    const originalDate = new Date('2025-12-15T10:00:00Z');
    ...
    const newDate = new Date('2025-12-20T14:00:00Z');
    ...
    expect(output.date.toISOString()).toBe(newDate.toISOString());
    expect(output.status).toBe(AppointmentStatus.scheduled);
  });

  it('should update appointment service successfully', async () => {
    // Arrange
    const { barber, barberShop } = await createBarberShopWithOwner();
  ...
  it('should not update an appointment from another barber shop', async () => {
  ...
  it('should throw BadRequestError when new date is not available', async () => {
    // Arrange
    const { barber, barberShop } = await createBarberShopWithOwner();
    const service = await createService(barberShop._id);
    ...
    const occupiedDate = new Date('2025-12-20T14:00:00Z');

    // Create appointment at occupied date
  ...
  it('should update appointment and verify in database', async () => {
    // Arrange
    const { barber, barberShop } = await createBarberShopWithOwner();
    const service = await createService(barberShop._id);
    const client = await createClient();
    ...
    const newDate = new Date('2025-12-25T16:00:00Z');
    ...
    expect(appointmentInDb.date.toISOString()).toBe(newDate.toISOString());
    expect(appointmentInDb.serviceId).toBe(service._id);
    expect(appointmentInDb.status).toBe(AppointmentStatus.scheduled);
  });

  it('should allow updating to an available time slot', async () => {
    // Arrange
    const { barber, barberShop } = await createBarberShopWithOwner();
    const service = await createService(barberShop._id);
    const client = await createClient();
    ...
    const availableDate = new Date('2025-12-22T13:00:00Z');
